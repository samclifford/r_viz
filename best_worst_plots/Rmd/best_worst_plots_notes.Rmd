...
Below, we see an example of a bar plot with additional chart junk. Specifically, we have: a 3D effect; too many additional labels, such as the total budget for each year; and arrows for labelling the estimate for the current year and recommendation for the next year.
...
By removing the elements in the below left, we retain the key information and are more easily able to make comparisons.

\begin{figure}
\includegraphics[width = \linewidth]{bars_omissions.jpg}
\caption{Elements of bar plot from p83 Tufte (1983)}
\end{figure}
